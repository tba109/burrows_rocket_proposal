...
while True:
    ...
    for n in range(16):
        print 'n = %d, x = %d' % (n,x)
        # buf = struct.pack('H'*65536,*[i for i in range(65536)])
        # buf = struct.pack('H'*1,*[8192*4])
        # buf = struct.pack('H'*4096,*[i for i in range(4096)])
        buf = struct.pack('H'*65536,*[int('{:016b}'.format(i)[::-1], 2) for i in range(65536)])
